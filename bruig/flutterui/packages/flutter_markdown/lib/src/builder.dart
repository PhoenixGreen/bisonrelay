...
class MarkdownBuilder implements md.NodeVisitor {
  ...
  bool visitElementBefore(md.Element element) {
    ...
    if (_isBlockTag(tag)) {
      ...
      if (_isListTag(tag)) {
        ...
        // Bison Relay: the header row gets a decoration of its own.
        //
        // Upstream gives the first row no decoration at all, so the row most
        // worth telling apart from the others was the one that could not be.
        // The striping below is unchanged: every other body row.
        BoxDecoration? decoration =
            styleSheet.tableCellsDecoration as BoxDecoration?;
        if (length == 0) {
          decoration = styleSheet.tableHeadDecoration;
        } else if (length.isOdd) {
          decoration = null;
        }
  ...
  void visitText(md.Text text) {
    ...
    if (_blocks.isNotEmpty && builders.containsKey(_blocks.last.tag)) {
      ...
      child = _buildRichText(
        TextSpan(
          // Bison Relay: the blockquote style is folded ON TOP of the
          // paragraph's, not underneath it.
          //
          // Upstream has these the other way round, which makes
          // styleSheet.blockquote dead in any sheet that fully specifies
          // `p` -- and every sheet built from a theme does. TextStyle.merge
          // lets the argument win, so blockquote.merge(p) is just p: a quote
          // could be given a colour, a size, a face or italics and none of
          // it reached the screen.
          style: _isInBlockquote
              ? (_inlines.last.style ?? const TextStyle())
                  .merge(styleSheet.blockquote)
              : _inlines.last.style,
  ...
  void _addBlockChild(Widget child) {
    ...
    if (parent.children.isNotEmpty) {
      // Bison Relay: list items are spaced apart from each other, blocks
      // from each other, and the two are allowed to differ.
      //
      // Upstream uses blockSpacing for both, and they want different
      // numbers: prose reads better with a clear gap between paragraphs,
      // and a list with that same gap between every bullet falls apart
      // into unrelated lines. Setting the one figure to the smaller of the
      // two was the other half of the bargain, and it left every heading,
      // quotation, code block and table sitting hard against whatever came
      // before it. See MarkdownStyleSheet.listItemSpacing.
      final double? spacing = parent.tag != null && _isListTag(parent.tag!)
          ? (styleSheet.listItemSpacing ?? styleSheet.blockSpacing)
          : styleSheet.blockSpacing;
      parent.children.add(SizedBox(height: spacing));
    }
