...
| `rule: false` | this group and the one under it are one subject split over two lines. Use it where the second group exists only because it needs a button of its own. |
...
| `canvasGroupGap` | 16 | each side of the rule between two groups |
