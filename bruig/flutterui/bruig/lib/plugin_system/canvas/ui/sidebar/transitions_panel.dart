    ...
    return SingleChildScrollView(
      ...
      child: CanvasControlScope(
        ...
        child: Column(
          ...
          children: [
            // Boxed, like every section of the settings panel: the box and the
            // room round it are what tells one section from the next. Bare
            // groups in a column sat a caption's height apart, which reads as
            // one long list rather than as sections.
            boxed(
              context,
              CanvasControlGroup(
                label: master
                ...
                children: [
                  ...
                  if (value.on)
                    CanvasDropdown<SceneTransitionKind>(
                      ...
                      width: 168,
                      options: [
                        for (var k in SceneTransitionKind.inFamily(
                            value.kind.familyOf))
                          (k, k.label)
                      ],
                  ...
                  if (value.on) ...[
                    CanvasNumberField(
                      ...
                      width: 58,
                      onChanged: (v) =>
                          write(value.copyWith(frames: v.round())),
                    ),
                    CanvasNumberField(
                      ...
                      width: 58,
                      onChanged: (v) =>
                          write(value.copyWith(overlap: v.round())),
                    ),
                    ...
                    if (value.kind.takesCount)
                      CanvasNumberField(
                        ...
                        width: 54,
                        onChanged: (v) =>
                            write(value.copyWith(count: v.round())),
                      ),
            ...
            // And the one that says where this transition came from. Not on
            // the shared canvas, where the transition being edited *is* the
            // one every scene comes from.
            if (!master)
              boxed(
                context,
