...
void paintChart(ui.Canvas canvas, Rect rect, ChartElement e,
  ...
  if (floating) {
    _placedLabel(canvas, rect, e.title, e.titleBox, defaultTitlePlacement,
        e.titleSpec);
    _placedLabel(
        canvas,
        rect,
...
Map<ChartLabelPart, Rect> chartLabelPlaces(ChartElement e, Rect rect) {
  ...
  return {
    if (e.titleBox.show && e.title.isNotEmpty)
      ChartLabelPart.title:
          _placeOf(e.titleBox, defaultTitlePlacement, rect),
    if (e.descriptionBox.show && e.description.isNotEmpty)
      ChartLabelPart.description: _placeOf(
          e.descriptionBox,
          defaultDescriptionPlacement(e.titleBox, e.title.isNotEmpty),
          rect),
...
_LegendLayout? _measureLegend(ChartElement e, Rect area, double reveal) {
  ...
  return _LegendLayout(rows, swatch, gap, rowHeight, between, rowGap,
      Size(width, height));
}
...
Rect _legend(ui.Canvas canvas, Rect area, ChartElement e, double reveal) {
  ...
  var left = switch (e.legend.placement) {
    ...
    LegendPlacement.left => Rect.fromLTRB(area.left + layout.size.width + pad,
        area.top, area.right, area.bottom),
    LegendPlacement.right => Rect.fromLTRB(area.left, area.top,
        area.right - layout.size.width - pad, area.bottom),
  };
