...
// new_post.dart is the Feed's own composer: a box to type a post in, a
// picture to put in it, and a button to publish it.
//
// It is deliberately plain, and deliberately does not grow. Everything that
// makes writing a post pleasant rather than merely possible -- the title, the
// preview, the drafts on disk, the marks under the words -- belongs to the
// Writing section, which exists only while the Writing Tools plugin is
// enabled (see plugin_system/writing_tools/ui/writing_screen.dart). With the
// plugin on, the Feed's New Post link goes there instead and this screen is
// not reached at all.
//
// The two do not share code, and that is the point. This one is what Bison
// Relay does without any plugin installed, and it should keep working exactly
// as it always has whatever happens to the other.
