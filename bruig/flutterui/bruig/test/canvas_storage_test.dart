...
void main() {
  ...
  group("what a stored picture is called", () {
    ...
    test("even when the drawing program wrote a page of preamble first", () {
      // The reported version: a picture added to a canvas that never
      // appeared, while another one added the same way did. A file written by
      // a drawing program opens with a declaration, a doctype, a licence
      // comment and a block of editor metadata, and the tag itself can be
      // most of a kilobyte in -- past where this used to stop looking. It was
      // then not a vector, and it is not a bitmap either, so nothing could
      // draw it at all.
      var preamble = "<!-- ${"Generated by a drawing program. " * 40} -->";
      expect(preamble.length, greaterThan(512),
          reason: "past the old limit, and well inside the new one");
      var svg = utf8.encode('<?xml version="1.0"?>\n$preamble\n'
          '<svg xmlns="http://www.w3.org/2000/svg"><rect/></svg>');
      expect(CanvasAssets.extensionForTest(svg), ".svg");
    });
